...
void MagicBeautify::_startBeauty() {
    LOGE("smoothlevel=%f---whitenlevel=%f", mSmoothLevel, mWhitenLevel);
    uint32_t *white;
    if (mWhitenLevel >= 1.0 && mWhitenLevel <= 5.0) {
        white = _startWhiteSkin(mWhitenLevel);
        if (mSmoothLevel >= 10.0 && mSmoothLevel <= 510.0) {
            _startSkinSmooth(white , mSmoothLevel);
        }
    } else{
        if (mSmoothLevel >= 10.0 && mSmoothLevel <= 510.0) {
            _startSkinSmooth(mImageData_rgb , mSmoothLevel);
        }
    }
}

uint32_t * MagicBeautify::_startWhiteSkin(float whitenlevel) {
...
uint32_t * MagicBeautify::_startSkinSmooth(uint32_t * pixels , float smoothlevel) {
    if (mIntegralMatrix == NULL || mIntegralMatrixSqr == NULL || mSkinMatrix == NULL) {
        LOGE("not init correctly");
        return NULL;
    }
    //磨皮肌肤
    Conversion::RGBToYCbCr((uint8_t *) pixels, mImageData_yuv, mImageWidth * mImageHeight);

    int radius = (int) (mImageWidth > mImageHeight ? mImageWidth * 0.02 : mImageHeight * 0.02);
